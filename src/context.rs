...
//!
//! GitHub Actions exposes the workflow context (repo, event payload,
...
pub struct GithubContext {
    ...
    /// True when running on Gitea Actions. Gitea's lock endpoint is
    /// wire-compatible with GitHub's, so the flag is informational here
    /// (drives a startup log line and routes through `GiteaClient` for
    /// structural parity with the action-pull-request-merge sister
    /// project), but it gives future Gitea-specific divergences a
    /// place to land.
    pub is_gitea: bool,
}
...
mod tests {
    ...
    #[test]
    fn parses_owner_repo_and_default_api_url() {
        with_env(
            &[
